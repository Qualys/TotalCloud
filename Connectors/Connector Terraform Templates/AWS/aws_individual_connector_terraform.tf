...
###################################################
# Attaching SecurityAudit and Custom Read Only Policy to the Qualys role
###################################################
...
resource "local_file" "authentication_key" {
  ...
  depends_on = [aws_iam_role_policy_attachment.system_defined_policy_attach,aws_iam_role_policy_attachment.custom_policy_attach]
}
module "QualysCloudViewAssetViewConnector" {
  source  = "matti/resource/shell"
  command = "curl -u '${var.username}:${var.password}' --header 'Content-Type: application/json' --header 'Accept: application/xml' -X POST --data-binary @- ${var.baseurl}/qps/rest/3.0/create/am/awsassetdataconnector < file.json"
  depends = [aws_iam_role_policy_attachment.system_defined_policy_attach,aws_iam_role_policy_attachment.custom_policy_attach]
}
...
output "ROLE_ARN" { value = aws_iam_role.QualysReadOnlyRole.arn}
